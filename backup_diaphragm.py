...
class BackupDiaphragm():
    ...
    def generar_pdf(self,fn):
        ...
        pdf.cell(75, 10, "Reporte ", 0, 1, 'C')
        pdf.set_font('arial', 'B', 8)
        # pdf.set_xy(70, 60)
        # pdf.cell(40, 10, "Lesion: "+self.Lesion, 0, 0, 'C')
        # pdf.set_xy(40, 70)
        ...
        if self.white_center != (0,0):
            ...
            if ((dist_izq+dist_der)-ancho_teorico < tolerancia_ancho):
                if abs(dist_izq - dist_der)> tolerance :
                    mensaje = "El punto blanco no se encuentra centrado " 
                    error = True
                else:
                ...
                new_row = {'Image':self.name_img, 'X1[mm]':distance - dist_izq, 'X2[mm]':distance - dist_der, 'ancho campo irradiado[mm]':(dist_izq+dist_der), 'error':error}
                ...
                new_row = {'Image':self.name_img, 'X1[mm]':distance - dist_izq, 'X2[mm]':distance - dist_der, 'ancho campo irradiado[mm]':(dist_izq+dist_der), 'error':"compruebe referencia"}
                self.df = self.df.append(new_row, ignore_index=True)
        else:
            mensaje = "Error encontrando el punto blanco"
            error = True
